...
async fn main() -> SampleResult<()> {
       ...
       // # Parallel requests with `Futures`
       {
              ...
              {
                     let futures: Vec<_> = urls.iter().map(|url| client.request(Method::GET, url.clone()).send()).collect();
       ...
       // - `DefaultDirectRateLiminter` & `DefaultKeyedRatelimiter` type aliases to allow easy passing around (!)
       {
              ...
              while let Some(result) = regulated_request_stream.next().await {
                     println!("{}: {:.2} {:.2}", count, start_time.elapsed().as_secs_f64(), call_time.elapsed().as_secs() as f64);
                     count += 1;
                     debug!(?result);
