...
impl PrintingActor {
        ...
        pub async fn handle_message(&mut self, msg: ActorMessage) -> Result<(), ActorError> {
                match msg {
                        ...
                        ActorMessage::GetPrintCount { resp_channel } => resp_channel
                                .send(self.print_count)
                                .unwrap(),
                        ActorMessage::GetStatus { resp_channel } => resp_channel
                                .send(self.active)
                                .unwrap(),
                }
                Ok(())
                                ...
                                if let Ok(idx) = n.parse::<usize>() {
                                        if let Some((tx, _)) = controls.get(idx) {
                                                ...
                                                std::io::stdin()
                                                        .read_line(&mut msg)
                                                        .unwrap();
                                                tx.send(ActorMessage::SetMessage(
                                                        msg.trim().to_string(),
                                                ))
                                                .await
                                                .unwrap_or_else(|e| {
