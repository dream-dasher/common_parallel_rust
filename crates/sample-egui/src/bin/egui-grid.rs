...
impl eframe::App for GridApp {
       fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
              egui::CentralPanel::default().show(ctx, |ui| {
                     ...
                     let grid_rect = Rect::from_min_size(available_rect.min + Vec2::new(10.0, 10.0), Vec2::new(grid_side, grid_side));

                     // Background for the grid
                     ui.painter().rect_filled(grid_rect, CornerRadius::ZERO, Color32::from_gray(240));
                     ...
                     for row in 0..self.grid.len() {
                            for col in 0..self.grid[row].len() {
                                   ...
                                   let cell_rect =
                                          Rect::from_min_size(egui::Pos2::new(cell_min_x, cell_min_y), Vec2::new(cell_size, cell_size));

                                   // Fill cell with color or black based on its state
                                   let fill_color = if self.grid[row][col] { self.cell_color } else { Color32::BLACK };
