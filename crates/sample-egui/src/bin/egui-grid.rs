...
fn main() {
    let native_options = eframe::NativeOptions { viewport: egui::ViewportBuilder::default().with_inner_size([800.0,
                                                                                                             600.0]),
                                                 ..Default::default() };

    eframe::run_native("Egui Grid Example",
                       native_options,
                       Box::new(|cc| {
...
impl eframe::App for GridApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
                                         ...
                                         let grid_side =
                                             (available_rect.width().min(available_rect.height()) - 20.0).max(100.0);
                                         let cell_size = grid_side / self.grid_size as f32;

                                         // Add some padding around the grid
                                         let grid_rect = Rect::from_min_size(available_rect.min
                                                                             + Vec2::new(10.0, 10.0),
                                         ...
                                         for i in 0..=self.grid_size {
                                             ...
                                             // Horizontal line
                                             ui.painter().line_segment([egui::Pos2::new(grid_rect.min.x, y),
                                                                        egui::Pos2::new(grid_rect.max.x, y)],
                                                                       Stroke::new(1.0, Color32::GRAY));

                                             // Vertical line
                                             ui.painter().line_segment([egui::Pos2::new(x, grid_rect.min.y),
                                                                        egui::Pos2::new(x, grid_rect.max.y)],
                                                                       Stroke::new(1.0, Color32::GRAY));
                                         }
                                         ...
                                         for row in 0..self.grid.len() {
                                             for col in 0..self.grid[row].len() {
                                                 ...
                                                 let cell_rect = Rect::from_min_size(egui::Pos2::new(cell_min_x,
                                                                                                     cell_min_y),
                                                 ...
                                                 ui.painter()
                                                   .rect_filled(cell_rect.shrink(1.0), // Shrink slightly to see the grid lines
                                                                CornerRadius::ZERO,
                                                                fill_color);
