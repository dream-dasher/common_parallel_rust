...
fn main() {
    ...
    eframe::run_native("AppName",
                       native_options,
                       Box::new(|cc| Ok(Box::new(AppStruct::new(cc))))).unwrap();
}

impl eframe::App for AppStruct {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ctx.set_pixels_per_point(2.0);
        egui::Window::new("My Window").default_open(true)
                                      .open(&mut self.open_var)
                                      // .constrain(false)
                                      .constrain_to(egui::Rect { min: pos2(10., 10.),
                                                                 max: pos2(1000., 200.), })
                                      .title_bar(false)
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
                                         ...
                                         if ui.button("toggle open_var")
                                              .clicked()
                                         {
                                             self.open_var = !self.open_var;
                                         };
                                         if ui.button("toggle interactable")
                                              .clicked()
                                         {
                                         ...
                                         ui.label(format!("interactable is: {}",
                                                          self.interactable));
                                         if ui.button("toggle enabledness")
                                              .clicked()
                                         {
