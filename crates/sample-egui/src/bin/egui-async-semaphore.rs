...
impl FuturesApp {
    ...
    fn metered_queue_request(&mut self, ctx: egui::Context) {
        ...
        let atomic_counter = self.request_tasks_to_create
                                 .clone();
...
impl eframe::App for FuturesApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::left("left_panel").show(ctx, |ui| {
            ...
            let mut request_period_ms = self.request_period
                                            .as_millis()
                                        as u64;
            if ui
                .add(
                    egui::Slider::new(&mut request_period_ms, 10..=10000)
                        .logarithmic(true)
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
                                         ...
                                         ui.add_space(10.0);
                                         ui.label(format!("Semaphore available: {}",
                                                          self.semaphore
                                                              .available_permits()));
                                         ui.label(format!("Queued Requests: {}",
                                                          self.join_set.len()));
                                         ui.label(format!("Queued Requests metered: {}",
                                                          self.join_set_caged
                                                              .lock()
                                                              .unwrap()
                                                              .len()));
                                         ui.label(format!("Request-Task Generators active: {}",
                                                          self.task_gen_tracker.len()));
                                         ui.label(format!("Request-Tasks to Create: {}",
                                                          self.request_tasks_to_create
                                                              .load(Ordering::Relaxed)));
                                         ui.add(
                egui::Slider::new(&mut self.delay_sec, 0..=10).text("Server Response Delay (sec)"),
            );
                                         ui.add(
                egui::Slider::new(&mut self.requests_to_queue, NON_ZERO_MIN..=NON_ZERO_MAX)
                    .logarithmic(true)
