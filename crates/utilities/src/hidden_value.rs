...
impl HiddenValue<std::string::String> {
    ...
    pub fn new_from_env<K>(/// Environment key to use to grab value to hide.
        ...
        let masked_string: Option<String> = if let Some(reveal_len) = reveal_len {
            ...
            if value.len() <= reveal_len {
                Err(HiddenValueError::RevealLengthTooLong { requested: reveal_len,
                                                            actual:    value.len(), })?
            }
            // last n chars (UTF-8)
            Some(value.chars()
        ...
        HiddenValue::builder().value(value)
...
impl<T> HiddenValue<T> {
    ...
    #[builder]
    pub fn new(/// Value to hide. (From accidental logging, printing, etc.)
               value: T,
               /// Optional String to use as an obfuscating debug representation of the value.
               ///
               /// As the value may be of an arbitrary type the caller must do any transforms or obfuscations
...
mod tests {
    ...
    fn test_partial_reveal() {
        ...
        let hidden = HiddenValue::builder().value(secret)
                                           .obf_string(TEST_OBF_STRING)
                                           .build()
                                           .unwrap();
        assert_eq!(format!("{:?}", hidden),
                   format!("HiddenValue {{ REDACTED..\"{}\" }}", TEST_OBF_STRING));
    }
    #[test]
    fn test_env_value() {
        ...
        let hidden = HiddenValue::from_env_builder().key(TEST_KEY)
                                                    .load_env_file(false)
                                                    .reveal_len(NonZeroUsize::new(4).unwrap())
                                                    .build()
                                                    .unwrap();
        ...
        assert_eq!(format!("{:?}", hidden),
                   format!("HiddenValue {{ REDACTED..\"{}\" }}", test_value_last_4));
    }
    #[test]
    fn test_reveal_length_too_long() {
        ...
        let result = HiddenValue::from_env_builder().key(TEST_KEY_2)
                                                    .load_env_file(false)
...
mod quickcheck_tests {
    ...
    fn qc_test_hidden_value_preserves_content(value: String) -> bool {
        let hidden = HiddenValue::builder().value(value.clone())
                                           .build()
                                           .unwrap();
    ...
    fn qc_test_reveal_length_validation(value_len: u16, reveal_len: Option<NonZeroUsize>) -> bool {
        ...
        match reveal_len {
            Some(reveal_len) => {
                ...
                let hidden = HiddenValue::from_env_builder().key(TEST_KEY_QC)
                                                            .load_env_file(false)
            ...
            None => HiddenValue::from_env_builder().key(TEST_KEY_QC)
                                                   .load_env_file(false)
                                                   .build()
                                                   .is_ok(),
        }
...
mod insta_tests {
    ...
    fn insta_test_hidden_value_result() {
        let hidden_result = HiddenValue::builder().value(12345)
                                                  .build();
