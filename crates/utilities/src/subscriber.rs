       ...
       let ((non_blocking_writer, trace_writer_guard), use_ansi) = match file_to_write_to {
              ...
              Some(file_path) => {
                     if let Some(parent) = file_path.parent() {
                            std::fs::create_dir_all(parent).expect("parent path should have been created if not already present");
                     }
                     (tracing_appender::non_blocking(std::fs::File::create(file_path).expect("log file should have been created")), false)
              }
       };
